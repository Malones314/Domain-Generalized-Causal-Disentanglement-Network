...
def process_section(csv_path, data_root):
    ...
    for _, row in tqdm(df.iterrows(), desc=f"处理 {os.path.basename(csv_path)}"):
        try:
            ...
            # 统一添加数据和标签
            domain_data[raw_data_type][domain]['data'].append(features)
            domain_data[raw_data_type][domain]['label'].append(label)
            domain_data[raw_data_type][domain]['d1p'].append(d1p)
            domain_data[raw_data_type][domain]['d1v'].append(d1v)
