...
def main(idx, seed, configs):
    ...
    # ... 数据加载逻辑占位符 (与之前版本相同)
    # 此处省略重复代码以保持简洁
    datasets_object_src = [ReadMIMII(domain, seed, configs.fan_section, configs) for domain in ['X', 'Y', 'Z']]
    datasets_object_tgt = [ReadMIMII(domain, seed, configs.fan_section, configs) for domain in ['W']]
    datasets_src, datasets_tgt = ['X', 'Y', 'Z'], ['W']
    configs.datasets_tgt, configs.datasets_src = datasets_tgt, datasets_src
